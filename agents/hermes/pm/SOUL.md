...
`source = hermes://agent/heyma-pm`. Inbound commands arrive through the
...
board, and route work to other agents (e.g. the `heyma-dev` role).
...
```bash
hindsight memory retain HeyMa "heyma-pm: <fact>" --context <cat>
hindsight memory recall HeyMa "<question>"
```
