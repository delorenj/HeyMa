...
| Before | After |
|---|---|
| `~/audio/inbox` (Syncthing **receiveonly**, 23 files) + `~/audio/ingest` (plain, 74 files) | **`~/audio/inbox` is a plain local dir and the only inbox.** `~/audio/ingest` is deleted. |
| Syncthing folder `id=audio` points at `~/audio/inbox` | Syncthing folder `id=audio` repointed to **`~/audio/dropoff`** (still receiveonly, still staggered/365d). `waxd` **copies** out of it, never writes into it. |
...
flowchart TB
  ...
  D["waxd — single owner<br/>flock ~/audio/var/waxd.lock"]
  D -->|systemd-run --user --scope| FF["ffmpeg -f pulse -c:a libopus"]
  FF --> P["~/audio/stream/&lt;rid&gt;.ogg.partial<br/>+ &lt;rid&gt;.rec.json  + &lt;rid&gt;.stop  + &lt;rid&gt;.fin.json"]
  P -->|"wait()==0 AND ffprobe&gt;0.5s<br/>THEN renameat2 RENAME_NOREPLACE"| IN["~/audio/inbox/YYYYMMDD-HHMMSS-&lt;slug&gt;.ogg"]

  DROP["~/audio/dropoff<br/>Syncthing receiveonly, 5 devices"] -->|copy2 to .staging, then renameat2| IN
  ...
  EP --> MD
  MD --> ARCHIVE["audio → ~/audio/archive/YYYY/MM/<br/>only after live S3 HEAD re-verify"]

  D --> LED[("~/audio/var/wax.db<br/>items · backups · transcripts<br/>passes · outbox · transitions")]
  ...
  D --> SOCK["~/audio/var/waxd.sock<br/>+ ~/audio/var/state.json mirror"]
```
...
|---|---|---|
| `ready` | No `~/audio/stream/*.rec.json`, no `*.ogg.partial`, no `*.stop`; preflight ok (`pactl get-default-source` non-empty **and** present in `pactl list short sources`; free bytes on `/dev/nvme0n1p2` ≥ 5 GiB — **426 G free at 88% today**; `~/audio/inbox` and `~/audio/stream` writable) | `bloodbank.evt.v1.audio.status.updated` |
...
Salvage on `error-partial`: rename in place to `<rid>.orphan`, `ffprobe`; if duration > 0, `ffmpeg -i <orphan> -c copy` a valid `.ogg` into inbox flagged `data.partial=true`; the orphan itself **moves** to `~/audio/recovered/orphans/` (never deleted, never left in `stream/`). Ogg/Opus is chosen precisely because a truncated Ogg stream is page-structured and remuxable — a truncated `.m4a` is a brick.

**Start always works.** `wax rec start` never refuses because of residue. It takes `~/audio/var/stream.lock`, mints a fresh rid, sweeps prior residue aside loudly (`session.failed` for the stranded rid), and records. A recorder that can be blocked by yesterday's crash is worse than the GUI app it replaces.

---

## State machine: `~/audio/inbox`
...
**A poison item does not wedge the pipeline.** A failed item stays in `~/audio/inbox` (so "dir non-empty AND pipeline not active" is literally true when idle), but the scheduler keeps claiming *other* pending items. `failed_count` rides on every status payload and forces YELLOW. `wax quarantine <id>` moves it to `~/audio/quarantine/` (never deleted).
...
wax status --json
curl -s --unix-socket /home/delorenj/audio/var/waxd.sock http://wax/status | jq .

# Individual machines
curl -s --unix-socket /home/delorenj/audio/var/waxd.sock http://wax/status/stream | jq .
curl -s --unix-socket /home/delorenj/audio/var/waxd.sock http://wax/status/inbox  | jq .

# COLD — daemon dead, nothing running, no state passed in.
# Pure function over ~/audio/stream + ~/audio/inbox + /proc. Exit code == severity.
wax state stream --cold --json
wax state inbox  --cold --json

# Degraded mirror (rewritten tmp+rename on every transition and every 5s)
jq . /home/delorenj/audio/var/state.json
```
...
|---|---|---|
| `~/audio/var/wax.db` (SQLite, WAL, single writer = the daemon) | **Authoritative.** Every query the pipeline makes. | local, µs |
...
`/home/delorenj/audio/etc/passes.d/<slug>.yaml`:
```yaml
slug: transcription-enhance
version: 1
argv: ["/home/delorenj/audio/bin/ep-transcription-enhance", "{md_path}"]
...
Icon mapping (precedence, evaluated on every generation bump): `stream==recording` → **RED** `wax-recording` (you must always be able to see you are recording, even if the pipeline is on fire); elif `stream ∈ {not-ready, error-partial, error}` OR `inbox ∈ {error, stopped}` OR `failed>0` OR `outbox_backlog>50` OR last S3 archive failed → **YELLOW** `wax-error`; else **GREEN** `wax-ready`. Tooltip carries both state strings + `cause_code`. New icon names in `~/.local/share/icons/hicolor/scalable/apps/wax-{ready,recording,error}.svg` (`#2ecc71` / `#e74c3c` / `#f39c12`) — **do not reuse the `vocalinux-*` names**, whose green/red semantics are *inverted* relative to yours (vocalinux: RED=idle, GREEN=listening). All GTK mutation from worker threads goes through `GLib.idle_add`.
...
|---|---|---|
| **1.** `watch_audio.sh` size-settle loop breaks after two equal 2s samples → relocated `record_0016.mp3` (951 MB, 16.5 h) mid-recording; transcribed as a 16-second stub. | The relay is **deleted**. `waxd` is the encoder's parent; the file moves only after `Popen.wait()==0` **and** `ffprobe -show_entries format=duration` > 0.5 s, via `renameat2(RENAME_NOREPLACE)` on the same filesystem (`stat -c %d` = **66306** for `~/audio`, `~/audio/inbox`, `~/d/Transcripts` — one fs, so the rename is genuinely atomic). No size-based heuristic exists anywhere in the codebase. Foreign writers land in `~/audio/dropoff` and are **copied** to `~/audio/inbox/.staging/<rid>.part` then renamed in. | `grep -rn 'stat -c %s\|st_size' /home/delorenj/code/wax/` → empty. Run `while :; do ls ~/audio/inbox; sleep 0.2; done` across a full 60 s recording — the file appears **exactly once, already complete**, never at partial size. |
| **2.** `trap 'rm -f "$LOG_FILE"' EXIT` at `bin/transcribe:219` destroys the log on **every** exit including failures. | Patch: `LOG_FILE="${TRANSCRIBE_LOG_FILE:-$(mktemp ...)}"` and install the trap **only** when the script allocated the temp itself. `waxd` passes `TRANSCRIBE_LOG_FILE=~/audio/var/logs/<item_id>/transcription.<attempt>.log`, retained 30 days, tail (8 KiB) shipped in `transcription.failed.data.stderr_tail`. | Force a failure (rename the model dir). Confirm `~/audio/var/logs/<item>/transcription.1.log` **survives**, and `curl 'http://127.0.0.1:8683/events?type=bloodbank.v1.audio.transcription.failed&limit=1' \| jq -r '.events[0].data.stderr_tail'` contains real whisper stderr — not the hardcoded string `transcription failed; see execution log`. |
...
- **Syncthing repoint, done first, not last:** stop syncthing → `mkdir ~/audio/dropoff` → move the *entire intact tree* (23 files **plus `.stfolder`**) from `~/audio/inbox` to `~/audio/dropoff` → edit `<folder id="audio">` path in `/home/delorenj/.local/state/syncthing/config.xml`, keeping `type=receiveonly` and `<versioning type="staggered" maxAge=31536000>` → restart. The `.stfolder` marker must exist at the new path or the folder errors and stops. The scar `~/audio/inbox/.stfolder.removed-20260629-053824` is still on disk — that reset destroyed a recording. **After this, `~/audio/inbox` is a plain local dir and it is safe to write to.**
- Snapshot: `tar --zstd -cf ~/audio/recovered/pre-wax-$(date +%F).tar.zst ~/audio ~/d/Transcripts`.
...
Files: `wax/tray.py`, three SVGs, `~/.config/systemd/user/waxd.service`, `wax-alert.service`.
...
Files: `wax/{ledger,machines/stream,machines/inbox,reconcile}.py`, `~/audio/var/wax.db`, `state.json` mirror with `generation`/`updated_at`/`daemon_pid`/`boot_id`.
Verify: drive every edge, assert on `sqlite3 ~/audio/var/wax.db 'select from_state,to_state,cause_code from transitions order by seq'`. Specifically: `fallocate` the disk under 5 GiB → `(ready,not-ready,disk_low)`, free it → back; drop a foreign mp3 into `~/audio/inbox` by hand with waxd running → a ledger row is minted within one tick and `inbox.pending` increments (**this is the INBOX IS INBOX test**); `kill -9` waxd, drop a file, restart → the file is discovered at boot.

**Phase 5 — Migration of the ~97 files.** *(~250 LOC, 0.5 day)*
Files: `wax/migrate.py`; manifest at `~/audio/var/migration-<ts>.json`.
Verify: before/after sha256 sets, `comm -23 before.sha after.sha` **empty**. `clip_0057.mp3` collision resolved to two distinct files (2,826,092 B and 9,658,988 B both present). `~/audio/ingest`, `outbox`, `processed` removed. `sqlite3 ... 'select state,count(*) from items group by state'` sums to 97 + the stray root `Xfinity_2026-01-22.mp3`, minus explicitly-reported exact-sha duplicates.
...
Heartbeat, nightly `mc cp ~/audio/var/wax.db delo/recordings/_ledger/`, log rotation, `wax reconcile --rebuild`. Candystore: audio summarizers + FilterBar chips + **rebuild the image** (`docker compose ... build candystore && up -d --force-recreate candystore` — it has no bind mounts).
Verify: `mv ~/audio/var/wax.db /tmp/ && wax reconcile --rebuild` reproduces items/backups/transcripts from S3 sidecars + vault frontmatter.

---

## Open decisions for you

1. **`~/audio/dropoff` vs. converting the Syncthing folder to sendreceive.** You said INBOX IS INBOX and I'm still handing you two directories. The receiveonly folder *reverts locally-added files* — that's the 2026-06-29 loss. **Recommendation: dropoff.** It's a *device feed*, not a second inbox: it never holds an item Wax hasn't already copied out, and it's one config line from deletion if the 5 sharing devices turn out to be dead. Alternative is sendreceive, which pushes multi-GB meeting recordings to your phone.
...
- `/home/delorenj/audio/inbox/clip_0057.mp3` ≠ `/home/delorenj/audio/ingest/clip_0057.mp3` (2.8 MB vs 9.6 MB). A bare `mv` in migration destroys one.
...
- Whether the 5 Syncthing devices sharing the audio folder still drop recordings at all, or whether `~/audio/dropoff` is pure legacy. If it's legacy, delete the folder and the relay and you're down to one directory.
